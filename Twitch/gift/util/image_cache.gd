...
func _init(do_disk_cache : bool, cache_path : String) -> void:
	...
	thread.start(Callable(self, "start"))
...
func start(params) -> void:
	...
	while active:
		if (!queue.is_empty()):
			...
			mutex.unlock()
			var buffer : PackedByteArray = http_request(entry.path, entry.type)
			if (disk_cache):
				if !d.dir_exists(entry.filename.get_base_dir()):
					d.make_dir(entry.filename.get_base_dir())
				f.open(entry.filename, FileAccess.WRITE)
...
func get_badge_mapping(channel_id : String = "_global") -> Dictionary:
	if !caches[RequestType.BADGE_MAPPING].has(channel_id):
		...
		if !disk_cache && file.file_exists(filename):
			...
			caches[RequestType.BADGE_MAPPING][channel_id] = test_json_conv.get_data()
			file.close()
		var buffer : PackedByteArray = http_request(channel_id, RequestType.BADGE_MAPPING)
		if !buffer.is_empty():
			var test_json_conv = JSON.new()
			test_json_conv.parse(buffer.get_string_from_utf8())["badge_sets"]
			caches[RequestType.BADGE_MAPPING][channel_id] = test_json_conv.get_data()
			if (disk_cache):
				file.open(filename, FileAccess.WRITE)
...
func http_request(path : String, type : int) -> PackedByteArray:
	var error := 0
	...
	if (host != new_host):
		error = http_client.connect_to_host(new_host)
