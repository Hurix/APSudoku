...
#	whisper("TEST", initial_channel)
	...
	add_command("su", Callable(get_parent(), "sudoku_input"), 2, 2)
	return success
...
func on_chat(data : SenderData, msg : String) -> void:
	#if %ChatContainer: %ChatContainer.put_chat(data, msg)
	pass
...
func no_permission(cmd_info : CommandInfo) -> void:
	print_debug("twitch issued cmd without permission: ", cmd_info)
#	chat("NO PERMISSION!")
